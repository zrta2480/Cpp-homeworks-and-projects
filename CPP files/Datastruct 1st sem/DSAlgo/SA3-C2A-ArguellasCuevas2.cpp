...
//Structs and Functions used for the manipulation of the Binary Search Tree
...
void addProduct(){ 
    ...
    do {
        ...
        cin >> productId;
        cinClearer();
        if (productId <= 0) //verifies the product ID inputted by the user
        {
...
void updateSalesFile(struct bstnode *root, int quantity){
    ...
    output  << left << setw(15) << root->productID << left << setw(30) << root->productDescrption << left << setw(30) << quantity << root->unitOfMeasurement << endl;
    output.close();
}
...
void updateOrderFile(struct bstnode *root, int quantity)
{
    ...
    output  << left << setw(15) << root->productID << left << setw(30) << root->productDescrption << left << setw(30) << quantity << endl;
    output.close();
}
...
void recieveProduct()
{
    ...
    {
        cout << "No Orders have been made..." << endl;
    }
    orderFile.close();
    int productId = 0;
    string idString;
    ...
    int tempQuantity = 0;
    int counter = 0;
    cout << "Enter the ID of product to recieve: "; cin >> productId; cout << endl;
    fstream orderReceive;
    orderReceive.open("orderFile.txt", ios::in);
    while (!orderReceive.eof())
    {
        if (counter > 0)
        {
            orderReceive >> idString >> tempDescription >> quantityString;
            tempId = stoi(idString);
            tempQuantity = stoi(quantityString);
            if (productId == tempId)
            {
                ProductIDFinder(tree, productId)->itemQuantity += tempQuantity;
                updateMasterList(tree, true);
            }
        }
        counter++;
